...
def _qntt_data_et(conn_id, user_addr, token_addr, col_str_dt, col_end_dt, ts_nodash, task_instance) :
    ...
    src_dt_amt_dict = src_df[["dt", "qntt_amt"]].set_index("dt").to_dict()["qntt_amt"]
    dt_ls = gen_daily_dt(str_dt, end_dt)
    res_df = pd.DataFrame(columns=["dt"], data=dt_ls)
    res_df["user_addr"] = user_addr
    res_df["token_address"] = token_addr
    res_df["qntt_amt"] = res_df["dt"].map(lambda x : src_dt_amt_dict[x] if x in list(src_dt_amt_dict.keys()) else 0)
    print(res_df)
    # sql문 만들기
    values = [f"('{row.dt}', '{row.user_addr}', '{row.token_address}', '{row.qntt_amt}')" for row in res_df.itertuples()]
